...
def validate_image_detections(image_detections):
    for i, image_detection in enumerate(image_detections):
        ...
        try:
            ...
            print("Image_detection: {}".format(image_detection))
            raise Exception("at index %d" % i)
        image = image_detection['image']
        for detection in image_detection['detections']:
            ## Following two adjustments are for DVIADetDB created via Gimp XCF flow
            if detection['right'] == image['width']:
                detection['right'] = image['width'] - 1
            if detection['bottom'] == image['height']:
                detection['bottom'] = image['height'] - 1

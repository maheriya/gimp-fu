        ...
        with open(detections_fpath) as f:
            ...
            for row in f_csv:
                ...
                detections.append({
                    'label': label,
                    'left': max(0.0, x1),
                    'right': min(1400, x2), ## shouldn't max out: images can be big. Original code maxed out at 1023
                    'top': max(0.0, y1),
                    'bottom': min(1400, y2) ## shouldn't max out: images can be big. Original code maxed out at 500
                })
        return detections
        ...
        for image_detection in image_detections:
            ...
            with open(out_labels_path, 'w') as csvfile:
                ...
                for detection in image_detection['detections']:
                    ...
                    x1 = "%4.2f"%detection['left']
                    x2 = "%4.2f"%detection['right']
                    y1 = "%4.2f"%detection['top']
                    y2 = "%4.2f"%detection['bottom']
